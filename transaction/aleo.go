...
func newTransferTransaction(account *account.Account, in *record.Record, ledgerProofs []string, amount, fee int64) (string, error) {
	...
	txn := C.new_transfer_transaction(inRecord, C.CString(ledgerProofs[0]), C.CString(ledgerProofs[1]), C.CString(account.PrivateKey().String()), C.int64_t(amount), C.int64_t(fee), C.CString(account.Address().String()))
